...
public class InventoryProvider extends ContentProvider {
    ...
    private Uri insertItem(Uri uri, ContentValues values) {
        ...
        if (price != null && price < 0) {
            throw new IllegalArgumentException("Price requires a valid format");
        }

        // If the weight is provided, check that it's greater than or equal to 0
        Integer quantity = values.getAsInteger(InventoryEntry.COLUMN_INVENTORY_QUANTITY);
        if (quantity != null && quantity < 0) {
            throw new IllegalArgumentException("Quantity requires a valid format");
        }

        // If the weight is provided, check that it's greater than or equal to 0
        Integer image = values.getAsInteger(InventoryEntry.COLUMN_INVENTORY_IMAGE);
        if (image != null ) {
    ...
    private int updateItem(Uri uri, ContentValues values, String selection, String[] selectionArgs) {
        ...
        if (values.containsKey(InventoryEntry.COLUMN_INVENTORY_PRICE)) {
            Integer price = values.getAsInteger(InventoryEntry.COLUMN_INVENTORY_PRICE);
            if (price == null && price < 0) {
                throw new IllegalArgumentException("Price requires a value");
            }
        }
        ...
        if (values.containsKey(InventoryEntry.COLUMN_INVENTORY_QUANTITY)) {
            ...
            Integer quantity = values.getAsInteger(InventoryEntry.COLUMN_INVENTORY_QUANTITY);
            if (quantity != null && quantity < 0) {
                throw new IllegalArgumentException("Item requires a valid weight");
            }
        }
